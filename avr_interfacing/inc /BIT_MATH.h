...
#define ROTATE_LEFT(Register, Bit)     (Register) = (((Register) << (Bit)) | ((Register) >> (BYTE_SIZE - (Bit))))

/** @brief Rotate an 8-bit Register right by Bit positions. */
#define ROTATE_RIGHT(Register, Bit)    (Register) = (((Register) >> (Bit)) | ((Register) << (BYTE_SIZE - (Bit))))
